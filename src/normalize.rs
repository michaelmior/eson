...
impl Normalizer {
    ...
    fn decomposed_tables(
        &self,
        tables: &mut HashMap<TableName, Table>,
        ...
        let t1_fields = t
            ...
            .clone()
            .into_iter()
            .filter(|&(ref k, _)| !vfd.rhs.contains(k))
            .map(|(k, v)| {
                (
                    k,
                    if v.key && vfd.rhs.contains(&v.name) {
                        Field {
                            name: v.name,
                            key: false,
        ...
        let t2_fields = t
            ...
            .clone()
            .into_iter()
            .filter(|&(ref k, _)| vfd.lhs.contains(k) || vfd.rhs.contains(k))
            .map(|(k, v)| {
                (
                    k,
                    if !v.key && vfd.lhs.contains(&v.name) {
                        Field {
                            name: v.name,
                            key: true,
                            cardinality: v.cardinality,
            ...
            for inds in schema.inds.values() {
                for ind in inds {
                    ...
                    let fds = right_table
                        .fds
                        .values()
                        .filter(|fd| fd.lhs.iter().all(|f| ind.right_fields.contains(f)))
                        .collect::<Vec<_>>();
                    let fd_fields = fds.iter().flat_map(|fd| fd.rhs.clone()).fold(
                        HashSet::new(),
                        |mut fields: HashSet<FieldName>, field| match ind
                            .right_fields
                            .iter()
                            .position(|f| f == &field)
                        {
                    ...
                    let remove_fields = ind
                        .left_fields
                        .iter()
                        .filter(|f| fd_fields.contains(*f) && left_table.fields.contains_key(*f))
                        .cloned()
                        .collect::<Vec<_>>();
                    ...
                    debug!(
                        "Removing {:?} from table {} because of {}",
        ...
        for inds in schema.inds.values() {
            for ind in inds {
                ...
                if left_table
                    ...
                    .fields
                    .keys()
                    .all(|f| ind.left_fields.contains(f))
                {
            ...
            for inds in schema.inds.values() {
                for ind in inds {
                    ...
                    let left_keys = ind
                        ...
                        .iter()
                        .enumerate()
                        .filter(|&(_, f)| left_table.fields[f].key)
                        .collect::<Vec<_>>();
                    let right_keys = ind
                        .right_fields
                        .iter()
                        .enumerate()
                        .filter(|&(_, f)| right_table.fields[f].key)
                        .collect::<Vec<_>>();
                    ...
                    if keys_match && schema.contains_ind(&ind.reverse()) {
                        ...
                        let mut new_table = Table {
                            name: format!("{}_{}", left_table.name, right_table.name)
                                .parse()
                                .unwrap(),
                            ..Default::default()
                        };
                        ...
                        for fd in left_table.fds.values() {
                            new_table.add_fd(
                                fd.lhs.iter().cloned().collect::<Vec<_>>(),
                        ...
                        for field in right_table.fields.values() {
                            ...
                            while new_table.fields.contains_key(&new_name) {
                                new_name =
                                    format!("{}{}", new_name, suffix).as_str().parse().unwrap();
                                suffix += 1;
                            }
                            new_right_names.insert(&field.name, new_name.clone());
                            new_table.fields.insert(
                                new_name.clone(),
                                Field {
                                    name: new_name,
                                    key: field.key,
                        ...
                        new_tables.push((
                            new_table,
                            ind.left_table.clone(),
                            ind.right_table.clone(),
                        ));
