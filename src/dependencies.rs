            ...
            for fd1 in self.values() {
                for fd2 in self.values() {
                    ...
                    if !self.contains_key(&lhs_copy)
                        || !new_fd.rhs.is_subset(&self.get(&lhs_copy).unwrap().rhs)
                    {
                        new_fds.push(new_fd);
                    }
                }
...
impl fmt::Display for IND {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        write!(
            f,
            "{}({}) <= {}({})",
            self.left_table, left_fields, self.right_table, right_fields
        )
    }
}
...
impl IND {
    ...
    pub fn is_subset(&self, other: &IND) -> bool {
        let left_indexes = self
            .left_fields
            .iter()
            .flat_map(|f| other.left_fields.iter().position(|f2| f2 == f))
            .collect::<Vec<_>>();
        let right_indexes = self
            .right_fields
            .iter()
            .flat_map(|f| other.right_fields.iter().position(|f2| f2 == f))
            .collect::<Vec<_>>();
        ...
        while changed {
            ...
            for inds in self.inds.values() {
                for (i, ind1) in inds.iter().enumerate() {
                    ...
                    let mut all_fields =
                        ind1.left_fields.clone().into_iter().collect::<HashSet<_>>();
                    ...
                    for (j, ind2) in inds.iter().enumerate() {
                        ...
                        if new_left
                            .iter()
                            .collect::<HashSet<_>>()
                            .is_subset(&all_fields.iter().collect::<HashSet<_>>())
                        {
                            continue;
                        }
                        ...
                        let new_ind = IND {
                            left_table: ind1.left_table.clone(),
...
mod tests {
    ...
    fn ind_is_subset() {
        ...
        let ind2 = IND {
            left_table: TableName::from("foo"),
            left_fields: field_vec!["bar", "qux"],
            right_table: TableName::from("baz"),
            right_fields: field_vec!["quux", "corge"],
        };
    ...
    fn ind_closure_transitive() {
        ...
        assert!(
            schema
                .inds
                .get(&(TableName::from("qux"), TableName::from("foo")))
                .len()
                > 0
        );
    }
    ...
    fn ind_closure_fd() {
        ...
        let inds = schema
            .inds
            .get(&(TableName::from("foo"), TableName::from("quux")));
        assert!(inds.len() == 1);
